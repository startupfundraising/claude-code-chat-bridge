...
2. Dial the other terminal:
   ...
   - If it returns `{"status": "error", "reason": "no_answerer"}`, tell the user: *"No terminal is in answer mode. Run `/ans` (Claude Code) or `$ans` (Codex) in the other terminal, then try again."* Stop.
